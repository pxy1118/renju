...
- Policy and value share the whole trunk; the policy head emits 225 logits (MCTS applies the softmax) and the value head is a `tanh` into `[-1, 1]`. The value head pools the trunk globally and then applies `128→64→1`: the original flattened `1×15×15` into 225 entries, which collapses to one dimension at batch size 1, and MCTS expands one leaf at a time.
- PUCT coefficient 2, 200 new searches per move; subtrees and existing visit counts are reused. Each node keeps, in order, all immediate wins, all forced fours (four in a row with both outer ends empty, so one stone cannot cover both ways to five), or all immediate blocks; otherwise it uses the `square3_line4` neighbourhood candidates. An empty board plays the centre only, and a node with no candidates falls back to every legal point. The three restricted sets are recorded as `forced_win`, `strategic`, and `forced_defense`.
