...
Output is a standard format-3 shard set, so it merges with the historical
teacher data through ``vk.datasets.combine_datasets`` and feeds the normal
pretraining path. The per-decision ``hard``/``regret`` flags stay out of the
...
from vk.datasets import FORMAT_VERSION, ShardWriter, sha256, _split   # noqa: E402
...
def model_move(game, evaluator, cfg, tree):
    if cfg["search"] == "policy":
        ...
        return policy_move(game, evaluator, cfg["candidates"])[0]
...
def collect(args):
    ...
    cfg = {"search": args.search, "candidates": args.candidates,
    ...
    with RapfiClient(args.engine, args.engine_dir, args.threads, args.hash_mb,
        ...
        for game_id in range(args.games):
            ...
            tree = MCTS(evaluator, cfg["simulations"], cfg["cpuct"],
                        candidates=cfg["candidates"]) if cfg["search"] == "mcts" else None
            annotated = 0
            while game.adjudicate() is None and positions < args.positions:
                ...
                writer.add(_split(game_id), {
                    "state": game.encode().astype(np.uint8),
                    "policy": teacher_policy(game, moves),
                    "value": np.float16(2 * best.winrate - 1),
                    "game_id": np.uint32(game_id),
                    "ply": np.uint16(np.count_nonzero(game.board)),
                    "teacher_best": np.uint16(best.action),
    ...
    values = np.concatenate([np.load(path, allow_pickle=False)["value"]
    ...
    manifest = {
        ...
        "collection": {"checkpoint": str(Path(args.checkpoint).resolve()),
                       "search": args.search, "candidates": args.candidates,
...
def main():
    ...
    parser.add_argument("--candidates", default="legal", choices=["tactical", "forced", "legal"])
