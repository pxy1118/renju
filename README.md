...
- PUCT 系数 2，每步 200 次新搜索；复用子树和已有访问次数。每个节点先保留全部一步胜或全部一步防，否则使用 `square3_line4` 邻域候选；空盘只走中心，候选为空才退回全部合法点。
...
runs/renju/                  连珠模型、回放和日志
  ...
  metrics.jsonl             时间、吞吐、损失、候选/搜索统计、分色胜率和晋级结果
...
默认随机启动配置每 10 轮进行一次 champion 晋级检验；混合配置每 5 轮检验。固定种子生成合法四手开局并交换执色，报告总胜负、分色结果、配对 Hoeffding 区间和得分 Wilson 区间。达到时间预算时不晋级，也不把半局计为和棋。
